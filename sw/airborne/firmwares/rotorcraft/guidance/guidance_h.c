...
struct Int32Vect2 gh_sps;
struct Int32Vect2 gh_sp3;
struct Int32Vect2 gh_sp2;
struct Int32Vect2 gh_sp1;
struct Int32Vect2 gh_sp0;
...
int32_t guidance_h_pngain;
int32_t guidance_h_dngain;
int32_t guidance_h_ingain;
int32_t guidance_h_nngain;
int32_t guidance_h_angain;
...
#ifndef GUIDANCE_H_NNGAIN
#define GUIDANCE_H_NNGAIN 0
#endif

#ifndef GUIDANCE_H_ANGAIN
#define GUIDANCE_H_ANGAIN 0
#endif
...
void guidance_h_init(void) {
  ...
  INT_VECT2_ZERO(gh_sps);
  INT_VECT2_ZERO(gh_sp3);
  INT_VECT2_ZERO(gh_sp2);
  INT_VECT2_ZERO(gh_sp1);
  INT_VECT2_ZERO(gh_sp0);
  ...
  guidance_h_pngain = GUIDANCE_H_PNGAIN;
  guidance_h_ingain = GUIDANCE_H_INGAIN;
  guidance_h_dngain = GUIDANCE_H_DNGAIN;
  guidance_h_nngain = GUIDANCE_H_NNGAIN;
  guidance_h_angain = GUIDANCE_H_ANGAIN;
...
__attribute__ ((always_inline)) static inline void  guidance_h_hover_run(void) {
  ...
  VECT2_COPY(gh_sp3,gh_sp2);
  VECT2_COPY(gh_sp2,gh_sp1);
  VECT2_COPY(gh_sp1,gh_sp0);
  VECT2_COPY(gh_sp0,ins_ltp_speed);
  
  VECT2_COPY(gh_sps,ins_ltp_speed);
  VECT2_ADD(gh_sps,gh_sp3);
  VECT2_ADD(gh_sps,gh_sp2);
  VECT2_ADD(gh_sps,gh_sp1);
    
  /* compute speed error    */
  //VECT2_COPY(guidance_h_speed_err, ins_ltp_speed);
  VECT2_SDIV(guidance_h_speed_err,gh_sps,4);
  ...
  /* run PID */
  // cmd_earth < 15.17
  guidance_h_command_earth.x = (guidance_h_pgain<<0)  * guidance_h_pos_err.x +
                                     guidance_h_dgain * (guidance_h_speed_err.x>>9) +
                                      guidance_h_igain * (guidance_h_pos_err_sum.x >> 12);
  guidance_h_command_earth.y = (guidance_h_pgain<<0)  * guidance_h_pos_err.y +
...
// 20 degres -> 367002 (0.35 << 20) increase slightly to fight wind better
#define NAV_MAX_BANK BFP_OF_REAL(0.45,REF_ANGLE_FRAC)
...
__attribute__ ((always_inline)) static inline void  guidance_h_nav_run(bool_t in_flight) {
  ...
  guidance_h_command_earth.x =
    guidance_h_pngain *  guidance_h_pos_err.x +
    guidance_h_dngain * (guidance_h_speed_err.x>>9) +
    guidance_h_ingain * (guidance_h_pos_err_sum.x >> 12) +
    guidance_h_nngain * guidance_h_nav_err.x +
    guidance_h_angain * guidance_h_accel_ref.x; /* feedforward gain */
  guidance_h_command_earth.y =
    guidance_h_pngain * guidance_h_pos_err.y +
    guidance_h_dngain * (guidance_h_speed_err.y>>9) +
    guidance_h_ingain * (guidance_h_pos_err_sum.y >> 12) +
    guidance_h_nngain * guidance_h_nav_err.y +
    guidance_h_angain * guidance_h_accel_ref.y; /* feedforward gain */
