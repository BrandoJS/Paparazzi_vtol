...
void guidance_v_run(bool_t in_flight) {
  ...
  if (in_flight) {
    // we should use something after the supervision!!! 
...
__attribute__ ((always_inline)) static inline void run_hover_loop(bool_t in_flight) {
  ...
  //low pass smoothing filter on err_zd
  err_zd = vd_tmp + ((err_zd-vd_tmp)>>3);
  vd_tmp = err_zd;
  ...
  Bound(guidance_v_ff_cmd,100,140); //Just in case of battery spikes
#endif
