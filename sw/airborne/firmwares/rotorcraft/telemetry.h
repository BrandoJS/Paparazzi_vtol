...
#define PERIODIC_SEND_BOOZ2_GPS(_chan) {				\
    DOWNLINK_SEND_BOOZ2_GPS( _chan,					\
			     ...
			     &booz_gps_state.heading,			\
			     &booz_gps_state.Gspeed,			\
			     &booz_gps_state.fix			\
			     );			\
  }
#else
...
#ifdef RCAM
#include "modules/ROSAM_CAM/rcam.h"
#include "modules/LISA_CAM_TRACK/lisa_ctrack.h"
#define PERIODIC_SEND_RCAM(_chan) {			\
    DOWNLINK_SEND_RCAM(_chan,				\
			&image_count,			\
			&point,				\
			&image_gps_lat			\
				   );		 	\
...
#ifdef AggieCap
#include "modules/AggieCap/aggie_cap.h"
#define PERIODIC_SEND_AGGIE_CAP(_chan){\
	DOWNLINK_SEND_AGGIE_CAP(_chan,			\
				&cam_on,		\
				&image_count,		\
				&image_phi,		\
				&image_the,		\
				&image_psi,		\
				&cam_psi_eulers.phi,	\
				&cam_psi_eulers.theta,	\
				&cam_psi_eulers.psi);			\
	}
#else
#define PERIODIC_SEND_AGGIECAP(_chan){}
#endif	

#ifdef Sonar
#include "modules/sonar/sonar_maxbotix_lisa.h"
#define PERIODIC_SEND_SONAR(_chan) {			\
	DOWNLINK_SEND_SONAR(_chan,			\
			    &sonar_meas,		\
			    &ins_sonar_offset,		\
			    &v1,			\
			    &v2,			\
...
#define PERIODIC_SEND_HGAINS(_chan){			\
	DOWNLINK_SEND_HGAINS(_chan,			\
				&guidance_h_pngain,	\
				&guidance_h_dngain,	\
				&guidance_h_ingain,	\
				&guidance_h_nngain,	\
