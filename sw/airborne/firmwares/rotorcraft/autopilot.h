...
#else
static inline void DetectGroundEvent(void) { /*if using sonar */
  if (autopilot_detect_ground_once) { /*only occurs if NavStartDetectGround is called*/
    if (sonar_meas < sonar_thresh) {
      autopilot_detect_ground = TRUE;
      autopilot_detect_ground_once = FALSE;
    }
  }
