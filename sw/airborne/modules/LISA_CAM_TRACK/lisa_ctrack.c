...
void init_lisa_ctrack(void) {
	...
	track_mode = 2;//TRACK_MODE_DEFAULT;
        foo = 0.;
	d_level = -45;
	x_star = 3;
	point = 90;
...
void periodic_lisa_ctrack(void)
{
  ...
  float alt = ins_enu_pos.z * 0.0039063;
  float x = x_star;
  switch (track_mode)
  {
    ...
    case 5:
	foo = 0;
	point = 90 - 180/3.1415*atan2(alt,x);
	level(theta,point);
	break;
...
void level(float theta,float desired){
	actuators_pwm_values[0] = SCALE0*(-desired+theta) + CENTER0;
}
