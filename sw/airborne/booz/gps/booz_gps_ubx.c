...
void booz_gps_ubx_read_message(void) {
  ...
  if (booz_gps_ubx.msg_class == UBX_NAV_ID) {
    if (booz_gps_ubx.msg_id == UBX_NAV_SOL_ID) {
      ...
      booz_gps_state.tow	= UBX_NAV_SOL_ITOW(booz_gps_ubx.msg_buf);
#ifdef GPS_LED
    ...
    } else if (booz_gps_ubx.msg_id == UBX_NAV_VELNED_ID) {
      booz_gps_state.vsd	= UBX_NAV_VELNED_VEL_D(booz_gps_ubx.msg_buf);
      booz_gps_state.Gspeed 	= UBX_NAV_VELNED_GSpeed(booz_gps_ubx.msg_buf);
      booz_gps_state.heading 	= UBX_NAV_VELNED_Heading(booz_gps_ubx.msg_buf);
